...
$.select = function (divBox, callback) {
    ...
    if (oBox.inited == undefined) oBox.inited = true; else return oBox;
    //3.数据初始化
    //------默认参数
    var args = {
        "selectkv": true,   //下拉框类型
...
$.checkbtn = function (o) {
    function switchChose(e) {
        ...
        //1.获取input
        var oc = this.getElementsByTagName("input")[0],op = $(this);
        if(new RegExp(/\b(disabled)\b/).test(this.className)) return;
        //2.(根据类型)切换状态
        var types = oc.getAttribute("type");
        var names = oc.getAttribute("name");
        var checked = true;
        if (types == "radio") {
            var oinputs = document.getElementsByName(names);//移除所有单选的选中样式
            for (var i = 0; i < oinputs.length; i++) $(oinputs[i].parentNode).removeClass("checked");
            op.removeClass("checked").addClass("checked");//选中当前单选
        }
        else if(op.hasClass("checked")) {
            op.removeClass("checked");
            oc.checked = false;
        }
        else op.addClass("checked");
        if($.getType(this.callback)==="function") this.callback();
        if (e && e.preventDeault) e.preventDeault();
    ...
    $(o).each(function () {
        if (this.inited !==true) {
            var self = $(this);
            var type = "checkbox", name = self.children("input:first").attr("name"),value = self.text();
            if(self.hasClass("radio-flat")) type = "radio";
            self.html(value+"<input type='"+type+"' name='"+name+"' value='"+value+"' />");
            if(self.hasClass("checked")) self.children("input:first").attr("checked",true);
            this.onclick = switchChose;//3.绑定事件
            this.inited = true;//4.标记组件已初始化实例
        }
...
//  -- node  对话框内容(必填 传入#id/.class/iframe)
...
//  -- AfterConfirm 确认后执行的函数(默认null)
    var obj = {
        ...
        Init: function (json) {
            ...
            //5.
            //添加iframe或页面内的对话框
            if(json.node===undefined || json.node==="" || json.node==="iframe") json.node = NewObject("iframe",{"frame-border":0,"class":"iframe-box"});
        ...
        Open: function () {
            ...
            if(this.node.nodeName.toLowerCase()==="iframe") this.node.src = this.url;
            this.oView.appendChild(this.node);
...
//6.选项卡组件 o必须是HTML Node
$.tabs = function(o, fn){
    o = o || null;
    if(o===null) { console.log("tabs is null!"); return; }
    if($.getType(fn)==="function") o.change = fn;
    if($.getType(o.inited)==="boolean") return o;
    else o.inited = true;
    o = $(o);
    ...
    headers.children("div").click(function(){
        var i=0,argContent=null;
        if(new RegExp(/\b(active)\b/).test(this.className)) return;
        ...
        if(this.index < contents.length) {
            argContent = contents[this.index];
            argContent.style["visibility"] = "visible";
            argContent.style["display"] = "block";
        }
        if(bar) bar.animate({"left":this.offsetLeft,"width":this.offsetWidth});
        if($.getType(o[0].change)==="function") o[0].change(this,argContent);
    });
}
//7.进度条组件 -m 内部的标签必须是 div表示进度 i用来显示文字 容器.progress .radius表示有圆角 .inner和.outer表示文字的位置 .red/.green/.blue表示可选的进度条颜色
$.progress = function(o, fn){
    //1.变量必须是Node节点
    if(o=== undefined || o===null || o.nodeType===undefined || o.nodeType != 1) {
        console.log(o);
        console.log("progress this error, variable is not Node!");
        return;
    }
    //回调 初始化后的进度条可以 重新初始化来更新回调函数
    if($.getType(fn)==="function") o.fn = fn;
    oc = $(o).children("div:first");
    var type = {t1: "left", t2: "width"};
    if($(o).hasClass("slider-vertical")) type = {t1: "top", t2: "height"};
    oc.css(type.t2,100*oc.css(type.t2)/$(o).css(type.t2)+"%");
    //2.初始化判断
    if($.getType(o.inited)!=="undefined") return o;
    o.inited = true;
    //3.事件
    //内部辅助函数 -- 百分比约束
    if($.getType(o.limit!=="function")) o.limit = function(percent){percent = percent < 0 ? 0 : (percent > 1 ? 1 : percent);percent = Math.floor(percent*100);return percent;}
    //内部辅助函数 -- 百分比显示
    if($.getType(o.showText)!="function") o.showText = function(txt){
        var self = $(this);
        var i = self.find("i:first");
        if(self.hasClass("inner")) i.removeAttr("class").text(txt);
        else if(self.hasClass("outer")){
            if(new RegExp(/^100/).test(txt)===true) i.html("").removeAttr("class").addClass("fa fa-check-circle fa-2x");
            else i.removeAttr("class").text(txt);
        }
    }
    //设置进度
    o.progress = function(percent1,percent2){
        percent1 =this.limit(percent1);
        percent2 =this.limit(percent2) - percent1;
        var self = $(this);
        var type = { "left": percent1+"%", "width": percent2+"%" };
        if(self.hasClass("slider-vertical")) type = { "top": percent1+"%", "height": percent2+"%" };
        self.children("div:first").css(type);
        this.showText(percent2+"%");
        if($.getType(this.fn)==="function") this.fn(percent1,percent2);
    };
    //设置进度 有动态效果
    o.animateProgress = function(percent1,percent2){
        var self = $(this);
        var type;
        percent1 = this.limit(percent1);
        percent2 = this.limit(percent2) - percent1;
        if(self.hasClass("slider-vertical")) type = { "top": this.offsetTop*percent1/100, "height": this.offsetHeight*percent2/100 };
        else type = { "left": this.offsetLeft*percent1/100, "width": this.offsetWidth*percent2/100 };
        self.children("div:first").animate(type,function(){
            if(self.hasClass("slider-vertical")) $(this).css({"top": percent1+"%","height": percent2+"%"});
            else $(this).css({"left": percent1+"%","width": percent2+"%"});
            self[0].showText(percent2+"%");
            if($.getType(self[0].fn==="function")) self[0].fn(percent1/100,percent2/100);
        });
    };
    return o;
}
/*8.滚动条 jquery e注意originalEvent*/
$.wheel = function(e){
    var bDown=true;
    bDown = e.originalEvent.wheelDelta ? e.originalEvent.wheelDelta < 0 : e.originalEvent.detail > 0;
    if(this.callback) this.callback(bDown);
...
$.drag = function(e){
    ...
    var oC =this;
    //var oscrollL = Math.max(document.documentElement.scrollLeft, document.body.scrollLeft); 
    //var oscrollT = Math.max(document.documentElement.scrollTop, document.body.scrollTop); 
    //y0为鼠标按下时距离obar顶部的距离
    var x0= e.clientX - oC.offsetLeft;
    var y0= e.clientY - oC.offsetTop;
    var res = {x:0,y:0};
    document.onmousemove = function(sEvt2){
        sEvt2 = window.event||sEvt2;
        res.x = sEvt2.clientX - x0;
        res.y = sEvt2.clientY - y0;
        oP.callback(res);
    }
...
$.scroll = function(o, fn){
    if(o===undefined || o===null || o.nodeType===undefined || o.nodeType!==1) {
        console.log(o);
        console.log("scroll bar error, o is not Node");
        return;
    }
    //回调
    if($.getType(fn)!=="undefined") o.callback = fn;
    if($.getType(o.inited)!=="undefined") return;
    o.inited = true;
    var self = $(o);
    o.bar = self.children("div:first")[0];
    $(o.bar).bind("mousedown",$.drag);
    self.bind("mousewheel",$.wheel);
    self.bind("DOMMouseScroll",$.wheel);
    o.onselectstart = function(){ return false; };
};
$.scrollBox = function(o,fn){
    var content = NewObject("div");
    var bar = NewObject("div");
    o.css("padding-right",o.css("padding-right"+12+"px"));
...
$(function () {
    ...
    if(BrowserDetect.browser=="Internet Explorer" && BrowserDetect.version < 9){
        $(".badge").each(function(){$(this).css("margin-right",Math.round(-this.offsetWidth/2)+"px");});
    }
    //进度条组件
